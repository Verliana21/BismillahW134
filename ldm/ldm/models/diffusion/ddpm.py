...
from ldm.modules.diffusionmodules.model import Encoder, Decoder, ResnetBlock, Upsample
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.model = DiffusionWrapper(unet_config, conditioning_key, LR_config)
    ...
    def log_images(self, batch, N=8, n_row=2, sample=True, return_keys=None, **kwargs):
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                samples, denoise_row = self.sample(cond=c,batch_size=N, return_intermediates=True)
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        super().__init__(conditioning_key=conditioning_key, LR_config = LR_config, *args, **kwargs)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        out = [z, c] # z:image,c:masked_image + mask
...
def apply_model(self, x_noisy, t, cond, x_start=None, return_ids=False):
    ...
    # Jika x_start tidak ada (saat inference), kita tidak bisa menjalankan logika BKRA.
    # Kita harus melewati BKRA dan langsung memanggil U-Net.
    if x_start is None:
        # Karena BKRA di-skip, tidak ada bgrec_loss.
        bgrec_loss = None 
        # Dapatkan conditioning dari input (tanpa modifikasi BKRA)
        c_concat = cond.get('c_concat', [])
        # Gabungkan input noisy (x_noisy) dengan conditioning
        xc = torch.cat([x_noisy] + c_concat, dim=1)
        # Langsung panggil U-Net (diffusion_model)
        x_recon = self.model.diffusion_model(xc, t)
    else:
        # Jika x_start ada (saat training), jalankan logika seperti biasa
        x_recon, bgrec_loss = self.model(x_noisy, t, x_start, **cond)
    # --------------------------------

    if isinstance(x_recon, tuple) and not return_ids:
        return x_recon[0], bgrec_loss # Pastikan mengembalikan 2 nilai
    else:
        return x_recon, bgrec_loss
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        model_output, bgrec_loss = self.apply_model(x_noisy, t, cond, x_start)
        ...
        if bgrec_loss is not None:
            ...
            loss = (loss_simple + bgrec_loss) / torch.exp(logvar_t) + logvar_t
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key, LR_config):
        super().__init__()
...
class BKRA(nn.Module):
    def __init__(self,
        ...
        bg_embed = torch.randn(embed_dim, n_embed)
